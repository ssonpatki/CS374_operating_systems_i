// encrypt server
// gcc enc_server.c -std=gnu99 -o enc_server to run
...
char* encryption(char *plaintext, char *key) {
	...
	for(int i = 0; i < len; i++) {
		...
		int plaintext_val = charToIndex(plaintext[i]);
		int key_val = charToIndex(key[i]);

		// sum plaintext and key integer values and wrap overflow
		cipher[i] = (plaintext_val + key_val) % 27;

		// map new integer value back to key to determine encrypted message
		encrypted_text[i] = indexToChar(cipher[i]);
	}
...
int main(int argc, char **argv) {
	...
	while (1) {
		...
		if (fork_result == 0) {
			...
			char* message_to_send = malloc(strlen(encrypted_message) + 3); // need to add @@ and null terminator
			strcpy(message_to_send, encrypted_message);
