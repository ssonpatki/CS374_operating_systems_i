// encrypt client
// gcc enc_client.c -std=gnu99 -o enc_client to run
...
// read plaintext from given file
    FILE* data_file = fopen(plaintext_file, "r"); 
	if (!data_file) { 
		fprintf(stderr, "Error opening file %s\n", plaintext_file);
		return 1;
	}
    
	// read text from plaintext file
	char* plaintext_line = NULL;
	size_t plaintext_buffer_size;
	ssize_t line_length = getline(&plaintext_line, &plaintext_buffer_size, data_file);
...
// create client socket
	...
	// check for any invalid characters in plaintext or key
	int plaintext_invalid = invalidChar(plaintext_line);
	int key_invalid = invalidChar(key_line);
	if (plaintext_invalid == 1) {
		fprintf(stderr, "Found invalid character in plaintext file\n");
		exit(1);
	...
	size_t plaintext_len = strlen(plaintext_line);
	size_t key_len = strlen(key_line);

	// exit if the key length is shorter than length of plaintext
	...
	snprintf(message_to_send, sizeof(message_to_send), "%s$%s$%s@@", client_name, plaintext_line, key_line);
